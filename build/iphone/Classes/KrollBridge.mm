...
@implementation FiveMinutesMoreObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_fiveminutesmore);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any FiveMinutesMore patches in JS before a context is loaded... nice for 
...
{
	// create FiveMinutesMore global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_fiveminutesmore = [[FiveMinutesMoreObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_fiveminutesmore];
	
	NSString *_fiveminutesmoreNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _fiveminutesmoreNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_fiveminutesmore valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.fiveminutesmore.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
